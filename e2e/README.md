...
| Focused current regression | `issue-283-visual-parity.spec.ts`, `issue-296-my-badges.spec.ts`, `issue-313-my-badges-layout.spec.ts` | `pnpm exec playwright test --config playwright.regressions.config.ts` |
...
current `main`. The #296 suite protects the current My/Badge interactions and
safe-area behavior. The #313 suite protects shared shell geometry, excludes
device chrome, and checks adaptive My spacing across supported viewport sizes.
These suites are intentionally non-gating because exact visual/layout assertions
require reconciliation against live Figma when Product changes.
