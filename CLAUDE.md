...
## Model Routing and Sol Escalation / モデル分担とSol相談

Use `deepseek-v4-flash` as the default implementation model when it is available in the current Claude Code environment. Sol is a selective reasoning resource, not the default implementer.

### Default behavior

- Do **not** ask Sol for a full implementation plan for every ticket.
- Start by reading the Issue, `AGENTS.md`, relevant Specs, nearby code, tests, and existing contracts.
- Let `deepseek-v4-flash` handle routine implementation, ordinary debugging, tests, mechanical refactors, and local UI/data changes.
- Keep implementation ownership with `deepseek-v4-flash` after a Sol consultation unless the task explicitly requires otherwise.

### Sol planning required before implementation

Ask Sol for a concise implementation plan **before editing** when the ticket materially involves one or more of the following:

1. Authentication, authorization, secrets, or other security-sensitive behavior.
2. Persistent schema/data-model migration or a shared public contract that is costly to reverse.
3. A cross-cutting architecture decision spanning multiple major subsystems.
4. A major refactor whose wrong direction would create substantial rework.
5. Infrastructure or deployment architecture with an irreversible or expensive choice.
6. A contradiction between the Issue, repository policy, and referenced Spec that changes product or engineering behavior.

The plan should cover only the decision, contract boundaries, implementation order, major risks, and verification strategy. Do not ask Sol to write routine implementation code.

### Escalation during implementation

Escalate a focused question to Sol when stronger reasoning is likely to avoid meaningful rework, especially when:

1. The root cause remains unexplained after **two focused investigation attempts using different hypotheses**.
2. There are multiple materially different approaches and repository evidence does not clearly determine the correct one.
3. Tests expose a systemic problem outside the expected ticket scope.
4. The apparent fix requires broad refactoring beyond the ticket boundary.
5. The proposed change affects security/privacy, persistent data meaning, shared contracts, or another high-risk boundary discovered during implementation.
6. Confidence in the root cause or intended contract is low enough that continuing would be guesswork.

Do not escalate ordinary lint/type errors, simple import mistakes, straightforward test failures, routine CRUD/UI work, or questions answerable by reading the repository.

### Sol escalation packet

Do not send Sol the entire task without synthesis. Provide the smallest useful context:

- Issue goal and relevant acceptance criteria.
- Relevant repository / Spec constraints.
- Files or areas already inspected.
- Current understanding and evidence.
- What has already been tried.
- Exact uncertainty or blocking question.
- Candidate approaches, if known.
- Relevant failing tests/errors.

Ask for the smallest useful answer, preferring one of:
...
After Sol answers, `deepseek-v4-flash` should resume implementation and verification.

If a mandatory Sol consultation is required by the rules above but no Sol consultation mechanism is available in the current environment, surface that limitation before making the high-risk edit instead of silently guessing.
